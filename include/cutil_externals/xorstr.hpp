 ...
 * Github: https://github.com/JustasMasiulis/xorstr
 *
...
#ifndef CUTIL_EXTERNAL_XORSTR_HPP__
#define CUTIL_EXTERNAL_XORSTR_HPP__
...
#include <cutil/cpp_base.hpp>
	...
	#include <immintrin.h>
#else
	#error xorstr: Unsupported platform!
#endif
...
#define CUTIL_XORSTR(str)  ::cutil::xorstr::xor_string(													\
								[]() { return str; }													\
								, std::integral_constant<std::size_t, sizeof(str) / sizeof(*str)>{}		\
								, std::make_index_sequence<::cutil::xorstr::detail::_buffer_size<sizeof(str)>()>{})
								
#define CUTIL_XORSTR_V(str) CUTIL_XORSTR(str).crypt_get()
/* Example:
	std::string var1 = CUTIL_XORSTR("Hello World!").crypt_get();
	std::string var2 = CUTIL_XORSTR_V("Hello World!");
	fmt::println("1={}, 2={}", var1, var2);
*/



#ifdef _MSC_VER
	#define CUTIL_XORSTR_FORCEINLINE __forceinline
#else
	#define CUTIL_XORSTR_FORCEINLINE __attribute__((always_inline)) inline
#endif

namespace cutil::xorstr { // C++17

	namespace detail {

		template<std::size_t Size>
		CUTIL_XORSTR_FORCEINLINE constexpr std::size_t _buffer_size()
		{
			return ((Size / 16) + (Size % 16 != 0)) * 2;
		}

		template<std::uint32_t Seed>
		CUTIL_XORSTR_FORCEINLINE constexpr std::uint32_t key4() noexcept
		{
		...
		CUTIL_XORSTR_FORCEINLINE constexpr std::uint64_t key8()
		{
		...
		CUTIL_XORSTR_FORCEINLINE constexpr std::uint64_t
		...
		CUTIL_XORSTR_FORCEINLINE std::uint64_t load_from_reg(std::uint64_t value) noexcept
		{
	...
	public:
		...
		template<class L>
		CUTIL_XORSTR_FORCEINLINE xor_string(L l, std::integral_constant<std::size_t, Size>, std::index_sequence<Indices...>) noexcept
			: _storage{ ::cutil::xorstr::detail::load_from_reg((std::integral_constant<std::uint64_t, detail::load_xored_str8<Size>(Keys, Indices, l())>::value))... }
		{}

		CUTIL_XORSTR_FORCEINLINE constexpr size_type size() const noexcept
		{
			return Size - 1;
		}

		CUTIL_XORSTR_FORCEINLINE void crypt() noexcept
		{
			// everything is inlined by hand because a certain compiler with a certain linker is _very_ slow
#if defined(__clang__)
			alignas(alignment)
				std::uint64_t arr[]{ ::cutil::xorstr::detail::load_from_reg(Keys)... };
			std::uint64_t*    keys =
				(std::uint64_t*)::cutil::xorstr::detail::load_from_reg((std::uint64_t)arr);
#else
			alignas(alignment) std::uint64_t keys[]{ ::cutil::xorstr::detail::load_from_reg(Keys)... };
#endif
		...
		CUTIL_XORSTR_FORCEINLINE const_pointer get() const noexcept
		{
			return reinterpret_cast<const_pointer>(_storage);
		}

		CUTIL_XORSTR_FORCEINLINE pointer get() noexcept
		{
			return reinterpret_cast<pointer>(_storage);
		}

		CUTIL_XORSTR_FORCEINLINE pointer crypt_get() noexcept
		{
